...
LORA_PATH = Path("models/qwen2.5-1.5b-task4-lora-real")

VAL_PATH = Path("data/lora/val_real_sft.jsonl")

BASE_OUTPUT = Path("runs/qwen2.5_1.5b_base_predictions.jsonl")
LORA_OUTPUT = Path("runs/qwen2.5_1.5b_lora_real_predictions.jsonl")
...
def clean_query(text: str) -> str:
    ...
    # Нам нужен только первый содержательный ответ.
    lines = [
        line.strip()
...
def main() -> None:
    ...
    # ---------------------------------------------------------
    # 2. Qwen с обученным LoRA-адаптером
    # ---------------------------------------------------------
    ...
    run_generation(
        ...
        run_name="qwen2.5_1.5b_lora_real",
    )
